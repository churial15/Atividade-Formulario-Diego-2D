...
if(mysqli_num_rows($seleciona) <= 0){
    ...
    while($res = mysqli_fetch_assoc($seleciona)){
        echo '<ul class="diego_formulario_2d">';
...
if(isset($_POST['atualiza'])){
    $idatualiza = intval($_POST['id']);
    $nome       = mysqli_real_escape_string($conexao, $_POST['nome']);
    $email      = mysqli_real_escape_string($conexao, $_POST
    ['email']);
    $msg        = mysqli_real_escape_string($conexao, $_POST['msg']);

    $sql = "UPDATE diego_formulario_2d SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
    mysqli_query($conexao, $sql) or die("Erro ao atualizar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM diego_formulario_2d WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM diego_formulario_2d WHERE id=$editar_id");
    $recado_editar = mysqli_fetch_assoc($res);
}
