...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE `diego-formulario-2d` SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM `diego-formulario-2d` WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM `diego-formulario-2d` WHERE id=$editar_id");
...
if(mysqli_num_rows($seleciona) <= 0){
    ...
    while($res = mysqli_fetch_assoc($seleciona)){
        echo '<ul class=`diego-formulario-2d`>';
