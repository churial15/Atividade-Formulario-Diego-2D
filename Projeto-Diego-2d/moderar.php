...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE `formulario-diego-2d` SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM `formulario-diego-2d` WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM `formulario-diego-2d` WHERE id=$editar_id");
...
if(mysqli_num_rows($seleciona) <= 0){
    ...
    while($res = mysqli_fetch_assoc($seleciona)){
        echo '<ul class=`formulario-diego-2d`>';
